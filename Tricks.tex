...
\[
  L(\delta) = \delta^\top H \delta = \begin{bmatrix}
    \delta_S \\
    \delta_{\bar S}
  \end{bmatrix}^{\!\top} \begin{bmatrix}
    H_{SS} & H_{S\bar S} \\
    H_{\bar S S} & H_{\bar S \bar S}
...
Expanding it, we get 
\[
  L(\delta) = \delta_S^\top H_{SS} \,\delta_S \;+\;
  2\,\delta_{\bar S}^\top H_{\bar S S}\,\delta_S \;+\;
  \delta_{\bar S}^\top H_{\bar S \bar S}\,\delta_{\bar S}.
\]
Now, notice that we can treat $\delta_{\cS}$ as constant (assume that we have already selected the coordinates in some way, which we will dicuss later, and because the values of those coordinates are fixed, they are constants), we can take the gradient w.r.t. to $\delta_{\bar{\cS}}$.  
