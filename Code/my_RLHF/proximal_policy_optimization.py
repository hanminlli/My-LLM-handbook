...
class PolicyWithValueHead(nn.Module):
    ...
    def token_loGRPObs(self, logits, labels):
        # logits: (B, T, V), labels: (B, T), returns loGRPOb of chosen labels per position
...
def compute_gae_with_bootstrap(rewards, values, mask, gamma, lam):
    ...
    rewards/values/mask: (B, T_full - 1) aligned to positions where policy/value/loGRPOb are defined.
...
def rollout(batch_prompts: List[str]):
    ...
    # Compute old loGRPObs/values (actor) and ref loGRPObs (reference)
    with torch.no_grad():
        ...
        old_loGRPObs_full = policy.token_loGRPObs(logits[:, :-1], labels_shifted) # (B, L_full - 1)
    ...
    batch = {
        ...
        "old_loGRPObs": old_loGRPObs_full,
        "ref_loGRPObs": ref_selected,
        "values": values_full,
...
def ppo_update(batch, kl_coef, target_kl=0.1):
    ...
    old_loGRPObs = batch["old_loGRPObs"].detach()   # (B, L_full-1)
    ref_loGRPObs = batch["ref_loGRPObs"].detach()   # (B, L_full-1)
    values_old = batch["values"].detach()           # (B, L_full-1)
    rewards = batch["rewards"]                      # (B, L_full-1)
    resp_mask = batch["resp_mask"]                  # (B, L_full-1)

    B, T = old_loGRPObs.shape # T = T_full - 1
    ...
    for epoch in range(args.ppo_epochs):
        ...
        for start in range(0, N, args.minibatch_size):
            ...
            cur_loGRPObs_full = policy.token_loGRPObs(cur_logits[:, :-1], labels_shifted) # (B, L_full - 1)
            cur_values = cur_value_full[:, :-1]
            cur_loGRPObs = cur_loGRPObs_full.reshape(-1)[sel]
            old_lp = old_loGRPObs.reshape(-1)[sel]
            ref_lp = ref_loGRPObs.reshape(-1)[sel]
            values_cur = cur_values.reshape(-1)[sel]
            adv_mb = adv_norm.reshape(-1)[sel]
            returns_mb = rets.reshape(-1)[sel]
        
            # PPO clipped objective
            ratio = torch.exp(cur_loGRPObs - old_lp)
            ...
            approx_kl = torch.mean(cur_loGRPObs - ref_lp)
    ...
    with torch.no_grad():
        ...
        cur_lp_full = policy.token_loGRPObs(logits_after[:, :-1], labels_shifted)
        approx_kl_full = (cur_lp_full - ref_loGRPObs)
