...
def initialize_distributed(
    ...
    """
    # Resets / clears any existing groups before creating new ones.
    parallel_state.destroy_model_parallel()
    
    # Torch setup for distributed training
    rank: int = int(os.environ["LOCAL_RANK"]) # Local rank
    world_size: int = torch.cuda.device_count() # Ranks in total
...
def get_train_data_iterator() -> Iterator:
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 0:
            # Compiles low-level C++ / CUDA helper extensions needed for data loading or tokenizer ops.
            # Example: fused kernels, dataset sampling helpers, etc
            # We only need to compile once per node (or even once globally), not on every rank.
            compile_helpers() 
        # Synchronization point: 
        # Every process must reach this line, None of them will proceed beyond it until all ranks have arrived.
    ...
    config: GPTDatasetConfig = GPTDatasetConfig(
        ...
        # Megatron sometimes needs slightly more data than the target count to fill all batches evenly across workers.
...
def save_distributed_checkpoint(
    ...
    # Ask the model to return only this rank’s portion (shard) of its state dictionary, instead of the full model parameters.
...
if __name__ == "__main__":
    ...
    # Creates the model object on the CPU (initially) in each process.
    # Each process has its own GPTModel object with the same structure, but each contains only the shard of weights belonging to that rank.
    gpt_model: GPTModel = model_provider()
    device: torch.device = torch.device("cuda")
    # This moves all model parameters and buffers from CPU to GPU memory — but only those that exist in this process.
    gpt_model.to(device)
    ...
    ddp_config: DistributedDataParallelConfig = DistributedDataParallelConfig(
        grad_reduce_in_fp32=False, # leave them in their original precision (usually FP16/BF16) when gradients are reduced
        overlap_grad_reduce=False, # Megatron will start the all-reduce as soon as each layer’s gradients are ready if set to yes
        use_distributed_optimizer=False, # Should Megatron use its distributed optimizer - 
        # (where optimizer states like momentum and variance are also sharded across GPUs) instead of a normal PyTorch optimizer?
    )
    ...
    # This retrieves the appropriate training scheduler function based on your current parallel configuration.
    # If we are not using pipeline parallelism - returns a simple sequential loop.
    # If we are using pipeline parallelism - returns a more complex schedule that passes activations between stages.
